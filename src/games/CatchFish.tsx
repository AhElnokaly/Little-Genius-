...
// +++ أضيف بناءً على طلبك: أنواع مختلفة من الكائنات البحرية +++
...
export default function CatchFish({ onBack, onWin }: CatchFishProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setFishes((prev) => {
        ...
        const emoji = MARINE_CREATURES[Math.floor(Math.random() * MARINE_CREATURES.length)];
  ...
  const catchFish = (id: number, e: React.PointerEvent) => {
    e.stopPropagation();
    if (onWin) onWin();
    ...
    setScore(s => {
      ...
      if (newScore % 5 === 0) {
        ...
        if ('speechSynthesis' in window) {
          ...
          const utterance = new SpeechSynthesisUtterance(`مرحلة ${level + 1}`);
    ...
    try {
      ...
      osc.frequency.exponentialRampToValueAtTime(1200, ctx.currentTime + 0.1); // +++ أضيف بناءً على طلبك: صوت إيجابي مميز +++
      ...
      <AnimatePresence>
        {fishes.map((fish) => (
          <motion.div
            ...
            onPointerDown={(e) => catchFish(fish.id, e)}
